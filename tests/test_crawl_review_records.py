...
def routes() -> dict[str, httpx.Response]:
    return {
        ...
        "https://culture.gov.taipei/News_Content.aspx?n=22311D615C1DFA8E&s=committee-one": response(
            '<a href="/files/committee.pdf">PDF</a>'
        ),
        ...
        "https://culture.gov.taipei/files/meeting-three.pdf": response(
            b"%PDF-1.7 third", "application/pdf"
        ),
        "https://culture.gov.taipei/files/committee.pdf": response(
            b"%PDF-1.7 committee", "application/pdf"
        ),
    }
...
@pytest.mark.parametrize(
    "value, expected",
    [
        ("115-7-1", date(2026, 7, 1)),
        ("115.07.02", date(2026, 7, 2)),
        ("115/7/03", date(2026, 7, 3)),
    ],
)
...
@pytest.mark.parametrize(
    "value", ["1-1-1", "115-07", "115-13-01", "115-02-30", "2026-07-01", "115_07_01"]
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "〖會議記錄〗105.07.28第10屆臺北市樹木保護委員會第16次專案小組暨第16次幹事會會議紀錄",
            True,
            False,
        ),
...
def test_safe_unicode_multi_attachment_immutable_and_manifest_consistency(tmp_path: Path) -> None:
    ...
    index = (
        f'<tr><td>115-07-01</td><td><a href="{detail}">樹木保護委員會幹事會{title}</a></td></tr>'
    )
    client = FakeClient(
        {
            ROOT: response(index),
            detail: response(f'<a href="{pdf}">PDF</a>'),
            pdf: response(b"%PDF-1.7 x", "application/pdf"),
        }
    )
...
def test_hash_duplicate_uses_existing_path_without_second_file(tmp_path: Path) -> None:
    ...
    one = "https://culture.gov.taipei/one"
    two = "https://culture.gov.taipei/two"
    pdf_one = "https://culture.gov.taipei/one.pdf"
    pdf_two = "https://culture.gov.taipei/two.pdf"
    ...
    crawl_records(
        ...
        FakeClient(
            {
                ROOT: response(first_index),
                one: response(f'<a href="{pdf_one}">PDF</a>'),
                pdf_one: response(payload, "application/pdf"),
            }
        ),
    )
    duplicate = crawl_records(
        ROOT,
        tmp_path,
        "review",
        FakeClient(
            {
                ROOT: response(second_index),
                two: response(f'<a href="{pdf_two}">PDF</a>'),
                pdf_two: response(payload, "application/pdf"),
            }
        ),
    )[0]
...
def test_cli_writes_exact_new_files_and_skips_unavailable_optional_source(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    config = tmp_path / "sources.json"
    config.write_text(
        json.dumps({"review_records": {"url": None, "required": False}}), encoding="utf-8"
    )
...
def test_hash_dedup_fails_closed_for_invalid_existing_manifest(
...
def test_existing_pdf_without_manifest_is_backfilled(tmp_path: Path) -> None:
    ...
    result = crawl_records(
        ...
        FakeClient(
            {
                ROOT: response(index),
                detail: response(f'<a href="{pdf_url}">PDF</a>'),
                pdf_url: response(payload, "application/pdf"),
            }
        ),
    )
...
def test_new_pdf_is_removed_if_manifest_write_fails(
    ...
    monkeypatch.setattr(
        crawler,
        "_ensure_manifest",
        lambda *_args: (_ for _ in ()).throw(ImmutableSnapshotError("manifest failed")),
    )
    with pytest.raises(ImmutableSnapshotError, match="manifest"):
        crawl_records(
            ROOT,
            ...
            FakeClient(
                {
                    ROOT: response(index),
                    detail: response(f'<a href="{pdf_url}">PDF</a>'),
                    pdf_url: response(b"%PDF-1.7 cleanup", "application/pdf"),
                }
            ),
        )
    assert not list(tmp_path.rglob("*.pdf"))
...
def test_pagination_accepts_only_positive_page_query_and_crawl_deduplicates_entries(
    tmp_path: Path,
) -> None:
    ...
    result = crawl_records(
        ...
        FakeClient(
            {
                ROOT: response(root_html),
                page_two: response(row),
                detail: response(f'<a href="{pdf_url}">PDF</a>'),
                pdf_url: response(b"%PDF-1.7 once", "application/pdf"),
            }
        ),
    )
    assert len(result) == 1
