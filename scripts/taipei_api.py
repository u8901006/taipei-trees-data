        ...
        try:
            ...
            if response.status_code not in _RETRYABLE_STATUS_CODES:
                if response.is_error:
                    raise RuntimeError(f"dataset metadata request failed with HTTP {response.status_code}")
                return response
            last_error = RuntimeError(f"dataset metadata request failed with HTTP {response.status_code}")
...
def _parse_resources(payload: Any) -> list[Resource]:
    ...
    for raw_resource in raw_resources:
        ...
        if not all(isinstance(value, str) and value.strip() for value in (identifier, name, resource_format, download_url)):
            continue
